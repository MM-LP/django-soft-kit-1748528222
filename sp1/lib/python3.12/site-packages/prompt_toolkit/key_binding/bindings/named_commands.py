...
def delete_char(event: E) -> None:
    ...
    """
    is_deleted = event.current_buffer.delete(count=event.arg)
    if not deleted:
...
def backward_delete_char(event: E) -> None:
    ...
    if event.arg < 0:
        ...
        is_deleted = event.current_buffer.delete_before_cursor(count=event.arg)
...
def kill_line(event: E) -> None:
    ...
    buff = event.current_buffer
    if event.arg < 0:
        is_deleted = buff.delete_before_cursor(
            count=-buff.document.get_start_of_line_position()
        )
    else:
        if buff.document.current_char == "\n":
            is_deleted = buff.delete(1)
        else:
            is_deleted = buff.delete(count=buff.document.get_end_of_line_position())
...
def kill_word(event: E) -> None:
    ...
    if pos:
        is_deleted = buff.delete(count=pos)

        if event.is_repeat:
            is_deleted = event.app.clipboard.get_data().text + deleted
...
def unix_word_rubout(event: E, WORD: bool = True) -> None:
    ...
    if pos:
        is_deleted = buff.delete_before_cursor(count=-pos)
